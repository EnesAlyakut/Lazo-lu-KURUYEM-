...
export default function EditKuponPage({ params }: PageProps) {
  ...
  useEffect(() => {
    fetch(`/api/kupon/${id}`)
      ...
      .catch(() => {
        toast.error("Kupon bilgileri alınamadı.");
  ...
  const submitCoupon = async (event: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/kupon/${id}`, {
        method: "PUT",
  ...
  return (
    <div className="max-w-3xl p-6 pt-20 lg:p-8 lg:pt-8">
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Kuponu Düzenle</h1>
          <p className="mt-1 text-sm text-gray-500">Kupon bilgilerini güncelleyin ve kaydedin.</p>
        </div>
      </div>
      ...
      <form onSubmit={submitCoupon} className="space-y-6">
        ...
        <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
          <DeleteCouponButton couponId={id} />
